...
public class AllTransactionsAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public class MyViewHolder extends RecyclerView.ViewHolder {
        ...
        public MyViewHolder(@NonNull View itemView) {
            ...
            tvAgain = itemView.findViewById(R.id.tv_again);
            ivRound=itemView.findViewById(R.id.iv_round);
    ...
    public AllTransactionsAdapter(CommonFunctions commonFunctions, boolean loadMore, Context con, List<TransactionsData> list, ItemClickAdapterListener itemClickAdapterListener) {
        this.list = list;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int i) {
        ...
        if (i == 0) {
            View v = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.all_transactions_list_item_style, viewGroup, false);
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, @SuppressLint("RecyclerView") final int i) {
        if (holder instanceof MyViewHolder) {
            ...
            if(debitOrCredit.equals("credit")){
                myViewHolder.tvAgain.setVisibility(View.GONE);
                ...
                if(list.get(i).getStatus().toLowerCase().equals(SthiramValues.SUCCESS)) {
                    myViewHolder.tvAgain.setVisibility(View.VISIBLE);
                }else{
                    myViewHolder.tvAgain.setVisibility(View.GONE);
                }
            ...
            //status color
            if(list.get(i).getStatus().toLowerCase().equals(SthiramValues.SUCCESS)) {
                myViewHolder.ivRound.setImageResource(R.drawable.ic_ellipse_green);
            }else if(list.get(i).getStatus().toLowerCase().equals(SthiramValues.PENDING)){
                message=con.getString(R.string.inprogress_on);
                myViewHolder.ivRound.setImageResource(R.drawable.ic_ellipse_yellow);
            }else{
                message=con.getString(R.string.cancelled_on);
                myViewHolder.ivRound.setImageResource(R.drawable.ic_ellipse_red);
            }
            ...
            myViewHolder.tvAgain.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    itemClickAdapterListener.sendAgain(view,i);
                }
            });
