...
import com.yeel.drc.api.login.LoginWithOTPResponse;
...
public class LoginOtpVerificationActivity extends BaseActivity {
    ...
    boolean otpVerificationFromLoginRetry=false;
    String countryCode;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        context = LoginOtpVerificationActivity.this;
    ...
    private void startSMSRetriever() {
        ...
        task.addOnSuccessListener(aVoid -> commonFunctions.logAValue("SMS Retriever","Success"));
        task.addOnFailureListener(e -> commonFunctions.logAValue("SMS Retriever","Failed"));
    }
    ...
    private void initView() {
        ...
        String mobileNumberFormat=apiCall.getCountyCodeFormatFromCountyList(commonFunctions.getCountryLists(),countryCode);
        String number= commonFunctions.formatAMobileNumber(mobileNumber,countryCode,mobileNumberFormat);
        ...
        }else tvMessage.setText(enter4DigitCodeInEn + " " + number);
    ...
    private void otpVerification() {
        apiCall.otpVerificationFromLogin(otpVerificationFromLoginRetry, mobileNumber,countryCode, otpId, otp, commonFunctions.getFCMToken(),true, new CustomCallback() {
            @Override
            public void success(CommonResponse response) {
                try {
                    String jsonString = apiCall.getJsonFromEncryptedString(response.getKuttoosan());
                    Gson gson = new Gson();
                    LoginWithOTPResponse apiResponse = gson.fromJson(jsonString, LoginWithOTPResponse.class);
                    if (apiResponse.getStatus().equalsIgnoreCase(SthiramValues.SUCCESS)) {
                        commonFunctions.setPIN("");
                        ...
                        }else if(apiResponse.getError_type().equals(SthiramValues.ERROR_TYPE_NEW_DEVICE)){
                            Intent intent = new Intent(context, ReturningUserActivity.class);
                            intent.putExtra("from-danger", true);
                            intent.putExtra("mobileNumber",mobileNumber);
                            intent.putExtra("countryCode",countryCode);
                            startActivity(intent);
                        }else {
                            if (apiResponse.getRetry_attempts() != null) {
            ...
            @Override
            public void retry() {
                otpVerificationFromLoginRetry=true;
                otpVerification();
    ...
    private void sendOtpAPICall(String type) {
        ...
        apiCall.sendOTP(type,sendOtpRetry,commonFunctions.getFCMToken(), mobileNumber,countryCode, otpId, true, new CustomCallback() {
            @Override
            public void success(CommonResponse response) {
                try {
                    String jsonString = apiCall.getJsonFromEncryptedString(response.getKuttoosan());
