...
public class DebitTransactionFragment extends Fragment {
    ...
    boolean getTransactionsByTypeRemainRetry=false;

    ShimmerFrameLayout shimmerFrameLayoutOne;

    public DebitTransactionFragment() {
        // Required empty public constructor
    }

    // TODO: Rename and change types and number of parameters
    public static DebitTransactionFragment newInstance(String param1, String param2) {
        DebitTransactionFragment fragment = new DebitTransactionFragment();
        return fragment;
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        view = inflater.inflate(R.layout.fragment_debit_transaction, container, false);
        context = getActivity();
        initView();
        setItemListeners();
        return view;
                ...
                if (dy > 0) {
                    ...
                    if (hasMore) {
                        if ((visibleItemCount + pastVisiblesItems) >= totalItemCount) {
                            hasMore = false;
                            getTransactionsByTypeRemainRetry=false;
    ...
    private void getTransactions(String from) {
        if(from.equals("")){
            if(!commonFunctions.getOutTransactions().equals("")){
                String jsonString=apiCall.getJsonFromEncryptedString(commonFunctions.getOutTransactions());
                Gson gson = new Gson();
        ...
        offset=0;
        apiCall.getTransactionsByType(getTransactionsByTypeRetry,commonFunctions.getUserAccountNumber(),"10",commonFunctions.getUserId(),"moneyOut",offset+"",false, new CustomCallback() {
            @Override
            public void success(CommonResponse response) {
                swipeRefreshLayout.setRefreshing(false);
                try{
                    commonFunctions.setOutTransactions(response.getKuttoosan());
    ...
    private void getTransactionsRemain() {
        apiCall.getTransactionsByType(getTransactionsByTypeRemainRetry,commonFunctions.getUserAccountNumber(),"10",commonFunctions.getUserId(),"moneyOut",offset+"",false, new CustomCallback() {
            @Override
            public void success(CommonResponse response) {
                try{
                    String jsonString=apiCall.getJsonFromEncryptedString(response.getKuttoosan());
            ...
            @Override
            public void retry() {
                getTransactionsByTypeRemainRetry=true;
                getTransactionsRemain();
            }
        });
    }
